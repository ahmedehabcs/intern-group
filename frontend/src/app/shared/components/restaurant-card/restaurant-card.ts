...
@Component({
  ...
  >
    <div class="aspect-[16/9] overflow-hidden bg-orange-50">
      <img
        appImageFallback
    ...
    <div class="p-4">
      <h3 class="truncate text-base font-medium text-slate-900">{{ restaurant().name }}</h3>
      <p class="mt-1 line-clamp-2 min-h-10 text-sm leading-5 text-slate-500">
        {{ restaurant().description }}
      </p>
      <div class="mt-3 flex flex-wrap gap-1.5">
        @for (category of restaurant().categories.slice(0, 3); track category) {
          <span class="rounded-md bg-orange-50 px-2 py-1 text-xs font-medium text-orange-700">{{
            category
          }}</span>
        }
      </div>
