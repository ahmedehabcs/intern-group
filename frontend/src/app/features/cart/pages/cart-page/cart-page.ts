...
@Component({ selector: 'app-cart-page', imports: [RouterLink, QuantitySelector, ImageFallbackDirective], templateUrl: './cart-page.html' })
export class CartPage {
  ...
  replaceConfiguration(item: CartItemResponse, specialInstructions: string): void {
    ...
    this.cartService.replace(item.id, {
      quantity: item.quantity,
      specialInstructions: specialInstructions.trim() || undefined,
      addons: item.addons.map(addon => ({ menuItemAddonId: addon.menuItemAddonId, quantity: addon.quantity })),
    }).pipe(finalize(() => this.processingId.set(null)), takeUntilDestroyed(this.destroy)).subscribe({
      error: () => this.error.set('Unable to update item configuration.'),
    });
  }
  async remove(id: number): Promise<void> {
    if (!await this.confirmation.confirm('Remove this item from your cart?', 'Remove item', 'Remove')) return;
    this.processingId.set(id);
    this.cartService
      .remove(id)
      .pipe(
  ...
  async clear(): Promise<void> {
    if (!await this.confirmation.confirm('This removes every item from your cart.', 'Clear cart', 'Clear cart')) return;
    this.cartService
      .clear()
