...
export class CartService {
  ...
  load(): Observable<CartResponse> {
    return (
      environment.mock.enabled ? this.mock.cart() : this.http.get<CartResponse>(this.api)
    ).pipe(tap((v) => this.cart.set(v)));
  }
  add(b: AddCartItemRequest): Observable<CartResponse> {
    return (
      environment.mock.enabled
        ? this.mock.addCartItem(b)
        : this.http.post<CartResponse>(`${this.api}/items`, b)
    ).pipe(tap((v) => this.cart.set(v)));
  }
  quantity(id: number, quantity: number): Observable<CartResponse> {
    return (
      environment.mock.enabled
        ? this.mock.cartQuantity(id, quantity)
        : this.http.patch<CartResponse>(`${this.api}/items/${id}/quantity`, { quantity })
    ).pipe(tap((v) => this.cart.set(v)));
  }
  replace(id: number, b: ReplaceCartItemRequest): Observable<CartResponse> {
    return (
      environment.mock.enabled
        ? this.mock.replaceCartItem(id, b)
        : this.http.put<CartResponse>(`${this.api}/items/${id}`, b)
    ).pipe(tap((v) => this.cart.set(v)));
  }
  remove(id: number): Observable<CartResponse> {
    return (
      environment.mock.enabled
        ? this.mock.removeCartItem(id)
        : this.http.delete<CartResponse>(`${this.api}/items/${id}`)
    ).pipe(tap((v) => this.cart.set(v)));
  }
  clear(): Observable<void> {
    return (
      environment.mock.enabled ? this.mock.clearCart() : this.http.delete<void>(`${this.api}/items`)
