...
import { CategoryResponse, MenuItemResponse, RestaurantResponse } from '../../../restaurants/models/restaurant.models';
...
export class Home {
  ...
  constructor() {
    this.api.categories().pipe(takeUntilDestroyed()).subscribe({ next: value => this.categories.set(value) });
  ...
  loadRestaurants(categoryId?: number): void {
    ...
    this.error.set(null);
    this.api.restaurants(categoryId)
  ...
  private loadDishes(restaurants: RestaurantResponse[]): void {
    const requests = restaurants.slice(0, 3).map(item => this.api.restaurant(item.id).pipe(catchError(() => of(null))));
    if (!requests.length) {
      this.dishes.set([]);
      return;
    }
    ...
    forkJoin(requests)
      .pipe(finalize(() => this.dishesLoading.set(false)), takeUntilDestroyed(this.destroyRef))
      .subscribe(details => this.dishes.set(details.flatMap(item => item?.menuSections.flatMap(section => section.menuItems).slice(0, 2) ?? []).slice(0, 6)));
  }
}
