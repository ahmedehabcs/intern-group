...
export const MOCK_DRIVER_PROFILE = {
  email: 'driver@talabaty.local',
  name: 'Youssef Ali',
  phoneNumber: '+20 111 400 2200',
...
export const MOCK_AVAILABLE_DELIVERIES = [
  {
    id: 2101,
    restaurantName: 'Firewood Pizza',
    deliveryAddress: '18 Dokki Street, Giza',
    itemCount: 3,
    totalPrice: 310,
    deliveryFee: 24,
  },
  {
    ...
    deliveryAddress: '7 Abbas El Akkad, Cairo',
    itemCount: 2,
    totalPrice: 245,
    deliveryFee: 22,
  },
  {
    ...
    deliveryAddress: '31 Lebanon Square, Giza',
    itemCount: 4,
    totalPrice: 380,
    deliveryFee: 28,
  },
...
export const MOCK_DELIVERY_HISTORY = [
  {
    id: 2099,
    restaurantName: 'Damascus Kitchen',
    deliveredAt: '2026-08-22T20:10:00Z',
    earnings: 38,
    orderTotal: 295,
  },
  {
    ...
    deliveredAt: '2026-08-21T17:30:00Z',
    earnings: 31,
    orderTotal: 185,
  },
  {
    ...
    deliveredAt: '2026-08-20T22:05:00Z',
    earnings: 42,
    orderTotal: 410,
  },
...
export const MOCK_KITCHEN_ITEMS = MOCK_KITCHEN_SECTIONS.flatMap(
  (section) => section.menuItems,
) satisfies KitchenMenuItemResponse[];
export const MOCK_KITCHEN_GROUPS = [
  {
    ...
    maxSelections: 1,
    addons: [
      { id: 1, name: 'Regular', additionalPrice: 0, available: true },
      { id: 2, name: 'Large', additionalPrice: 25, available: true },
    ],
  },
  {
    id: 2,
    ...
    maxSelections: 3,
    addons: [
      { id: 3, name: 'Extra cheese', additionalPrice: 12, available: true },
      { id: 4, name: 'Jalapeños', additionalPrice: 8, available: true },
      { id: 5, name: 'Crispy onions', additionalPrice: 10, available: true },
    ],
  },
...
export const MOCK_KITCHEN_DASHBOARD = {
  ...
  cancelledOrders: 1,
  delayedOrders: 1,
} satisfies KitchenDashboardSummaryResponse;

export const MOCK_ADMIN_CATEGORIES = MOCK_CATEGORIES.map((category) => ({
  ...category,
  active: true,
...
export const MOCK_ADMIN_RESTAURANTS = MOCK_RESTAURANTS.map((restaurant) => ({
  id: restaurant.id,
  name: restaurant.name,
  phone: `+20 100 100 10${restaurant.id.toString().padStart(2, '0')}`,
  email: `contact${restaurant.id}@talabaty.local`,
  address: `${10 + restaurant.id} Marketplace Street`,
  governorateName: restaurant.id % 2 ? 'Cairo' : 'Giza',
  description: restaurant.description,
  logoUrl: restaurant.logoUrl,
  isActive: true,
  deliveryFee: 18 + restaurant.id,
  categoryNames: restaurant.categories,
})) satisfies RestaurantAdminResponse[];
export const MOCK_ADMIN_ORDERS = MOCK_CUSTOMER_ORDERS.map((order, index) => ({
  id: order.id,
  status: order.status,
  restaurantName: order.restaurantName,
  customerName: MOCK_ADMIN_CUSTOMERS[index % MOCK_ADMIN_CUSTOMERS.length].name,
  riderName: order.status === 'PENDING' ? null : 'Youssef Ali',
  totalPrice: order.totalPrice,
  deliveryFee: order.deliveryFee,
  updatedAt: order.createdAt,
})) satisfies OrderAdminResponse[];
export const MOCK_RIDERS = [
  ...
  },
  {
    id: 2,
    name: 'Ahmed Samir',
    phoneNumber: '+20 106 222 9911',
    vehicleType: 'Bicycle',
    licenseNumber: 'BI-11820',
    nationalId: '30002021234568',
    approvalStatus: 'APPROVED',
    online: false,
    isActive: true,
  },
  {
    id: 3,
    name: 'Mahmoud Fathy',
    phoneNumber: '+20 115 555 8140',
    vehicleType: 'Motorcycle',
    licenseNumber: 'DL-90012',
    nationalId: '29709091234561',
  ...
  },
  {
    id: 4,
    name: 'Hany Mostafa',
    phoneNumber: '+20 120 321 9010',
    vehicleType: 'Car',
    licenseNumber: 'CR-33510',
    nationalId: '29503111234566',
    approvalStatus: 'PENDING',
    online: false,
    isActive: true,
  },
  {
    id: 5,
    name: 'Tamer Eid',
    phoneNumber: '+20 102 909 1200',
    vehicleType: 'Motorcycle',
    licenseNumber: 'DL-74321',
    nationalId: '29207071234562',
    approvalStatus: 'REJECTED',
    online: false,
    isActive: false,
  },
] satisfies RiderAdminResponse[];
export const MOCK_ADMIN_FEEDBACK = MOCK_DRIVER_FEEDBACK.map((feedback) => ({
  ...feedback,
